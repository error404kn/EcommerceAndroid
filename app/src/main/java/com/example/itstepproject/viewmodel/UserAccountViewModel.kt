...
import com.example.itstepproject.EcommerceApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: User, imageUri: Uri?){
        val areInputsValid = validateEmail(user.email) is RegisterValidation.Success
                && user.firtstName.trim().isNotEmpty()
                && user.lastName.trim().isNotEmpty()

        if (!areInputsValid){
            viewModelScope.launch {
                _user.emit(Resource.Error("შეამოწმეთ თქვენი ველები"))
            }
            return
        }
        viewModelScope.launch {
        ...
        viewModelScope.launch {
            try {
                val imageBitMap = MediaStore.Images.Media
                    .getBitmap(getApplication<EcommerceApplication>().contentResolver, imageUri)
